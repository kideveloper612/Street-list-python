...
csv_header = [['Store Address', 'Title', 'Alexa', 'Best Selling', 'Country']]
...
def main():
    base_url = 'https://www.shopistores.com/shopify-jewelry-stores/'
    flag = True
    page = 0
    while flag:
        page += 1
        ...
        if response is not None:
            ...
            for soup_record in soup_records:
                ...
                selling = soup_record.find('td', attrs={'data-title': 'Best Selling'}).a['href']
                country = soup_record.find('td', attrs={'data-title': 'Country'}).text.strip()
                line = [address, title, rank, selling, country]
                print(line)
                write_csv(lines=[line], filename='jewelry.csv')
        else:
            flag = False
